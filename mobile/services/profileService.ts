...
export const profileService = {
    ...
    async getStats(organizationId: string) {
        ...
        const totalRevenue = (revenueStats.data || []).reduce((sum: number, p: any) => sum + Number(p.amount), 0);
    ...
    async getGraduationStatus(programId: string, userId: string) {
        ...
        const attendedCount = attendance?.filter((a: any) => a.status === 'present').length || 0;
